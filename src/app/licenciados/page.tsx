  ...
  return (
    <main className="bg-slate-50">
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="overflow-hidden rounded-[2rem] bg-gradient-to-br from-violet-50 to-fuchsia-200 p-8 shadow-2xl ring-1 ring-slate-200 sm:p-10">
          <div className="grid gap-8 lg:grid-cols-[1.5fr_1fr] lg:items-end">
            <div className="max-w-2xl">
              <span className="inline-flex rounded-full bg-fuchsia-100 px-3 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-fuchsia-700">
                Licenciados
              </span>
              <h1 className="mt-6 text-4xl font-bold tracking-tight text-slate-900 sm:text-5xl">
                Personagens, conforto e estilo para os seus pés
              </h1>
              <p className="mt-4 max-w-xl text-lg leading-8 text-slate-600">
                Descubra calçados licenciados com design exclusivo e acabamento premium para marcar presença todos os dias.
              </p>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col gap-4 rounded-3xl bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-8">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-fuchsia-700">Licenciados</p>
            <h2 className="mt-3 text-3xl font-semibold text-slate-900">Navegue pelos melhores modelos</h2>
            <p className="mt-2 text-sm text-slate-500">{sortedProducts.length} opções disponíveis para colecionar estilo e conforto.</p>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="inline-flex rounded-full bg-slate-100 p-2 text-sm text-slate-600 shadow-inner">
              ...
              <select
                ...
                className="ml-3 rounded-full border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 outline-none transition focus:border-fuchsia-500 focus:ring-2 focus:ring-fuchsia-100"
              >
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <Link
                  key={item.label}
                  href={item.href}
                  className="rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-fuchsia-300 hover:bg-fuchsia-50"
                >
                  {item.label}
                </Link>
              ))}
        ...
        <div className="flex justify-around items-center p-4 w-full h-full mt-10">
          ...
          <div className="flex flex-col gap-4">
            ...
            >
              <p className="text-sm uppercase tracking-[0.3em] text-fuchsia-300">Feminino</p>
              <p className="mt-4 text-xl font-semibold">Conforto para elas</p>
            </Link>
            <Link
              href="/kids"
