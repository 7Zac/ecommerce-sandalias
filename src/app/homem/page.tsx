    ...
    <main className="bg-slate-50">
      ...
      <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-col gap-4 rounded-3xl bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between sm:p-8">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-cyan-700">Homem</p>
            <h2 className="mt-3 text-3xl font-semibold text-slate-900">Navegue pelos melhores modelos</h2>
            <p className="mt-2 text-sm text-slate-500">{sortedProducts.length} opções disponíveis com estilo e durabilidade.</p>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="inline-flex rounded-full bg-slate-100 p-2 text-sm text-slate-600 shadow-inner">
              ...
              <select
                ...
                className="ml-3 rounded-full border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 outline-none transition focus:border-cyan-500 focus:ring-2 focus:ring-cyan-100"
              >
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <Link
                  key={item.label}
                  href={item.href}
                  className="rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-cyan-300 hover:bg-cyan-50"
                >
                  {item.label}
                </Link>
              ))}
